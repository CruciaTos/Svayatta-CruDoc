...
class _RevenueScreenState extends State<RevenueScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.transparent,   // transparent – global gradient from shell shows through
      body: SafeArea(   // no extra gradient container
        child: Padding(
            ...
            children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Last Paid: ${lastPaid.payer}', style: const TextStyle(color: AppColors.textPrimary, fontSize: 16, fontWeight: FontWeight.w600)),
                            const SizedBox(height: 4),
                            Text('${lastPaid.description} • ${DateFormat.yMMMd().format(lastPaid.date)}', style: const TextStyle(color: AppColors.textSecondary, fontSize: 13)),
                          ],
                        ),
                      ),
                      Text('\$${lastPaid.amount.toStringAsFixed(0)}', style: const TextStyle(color: AppColors.beige, fontSize: 18, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
              const SizedBox(height: 16),
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text('\$${totalRevenue.toStringAsFixed(0)}', style: const TextStyle(color: AppColors.beige, fontSize: 20, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
              const SizedBox(height: 16),
                ...
                children: ['Weekly', 'Monthly', 'Yearly', 'All'].map((filter) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Text(
                        filter,
                        style: TextStyle(color: isSelected ? AppColors.midnightBlue : AppColors.textSecondary, fontSize: 13, fontWeight: FontWeight.w500),
                      ),
                    ),
                  );
                }).toList(),
              ...
              Expanded(
                child: filtered.isEmpty
                    ? const Center(child: Text('No revenue entries for this period', style: TextStyle(color: AppColors.textSecondary)))
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          IconData icon;
                          switch (entry.type) {
                            case RevenueType.visit: icon = Icons.medical_services; break;
                            case RevenueType.online: icon = Icons.videocam; break;
                            case RevenueType.miscellaneous: icon = Icons.miscellaneous_services; break;
                          }
                          return Container(
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Text(entry.description, style: const TextStyle(color: AppColors.textPrimary, fontSize: 15, fontWeight: FontWeight.w600)),
                                      const SizedBox(height: 4),
                                      Text(DateFormat.yMMMd().format(entry.date), style: const TextStyle(color: AppColors.textSecondary, fontSize: 13)),
                                    ],
                                  ),
                                ),
                                Text('\$${entry.amount.toStringAsFixed(0)}', style: const TextStyle(color: AppColors.beige, fontSize: 16, fontWeight: FontWeight.w600)),
                              ],
                            ),
                          );
                        },
